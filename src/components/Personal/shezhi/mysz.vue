		...
		<div class="jubao" v-show="show">
			<div class="jubaoall">
				...
				<div class="a">
					...
					<a class="a2" @click="loginOut">确定</a>
				</div>
			</div>
		</div>
	</div>
...
export default {
	...
	methods: {
    isHaveMobile(){
      this.$http.get(getOpenUrl() + "/user/mobile", {
//        params:{
...
//        },
        ...
        if(res.body.status){
          //telyz
          if(res.body.data.is_mobile===0){
            this.$router.push({path: '/newtelyz'});
          }else{
            this.$router.push({path: '/telyz'});
          }
        }
      });
    },
		...
		loginOut(){
			this.$http.get(getOpenUrl() + "/Login/goOut", {
        headers: {
          'Authorization': 'Bearer ' +  getToken()
        }
			}).then((res)=>{
          ...
          localStorage.removeItem('token');
					setCookie('token', "", -1);
					setCookie('islogin', "", -1);
					setCookie('isexpert', "", -1);
					setCookie('ishasgroup', "", -1);
          setCookie('groupview', "", -1);
          setCookie("findview", "", -1);
...
<style scoped>
  ...
  .body-list{border-bottom: 0.01rem solid #ebebeb;overflow: hidden;}
  .body-list li{float: left;height: 0.87rem;line-height: 0.87rem;font-size: 0.22rem;color: #242424;}
  ...
  .body-list .exit{background: url(../../../images/other/tuichudenglu.png)no-repeat 0 50%;background-size:0.26rem;}
